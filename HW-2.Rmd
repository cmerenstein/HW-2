...
pa <- pa + geom_abline(intercept=b[1], slope=b[2], col="#F8766D", size=1)
...
```
> doesn't look linear, residuals look normally distributed in the y but not the x.
...
pb <- pb + geom_abline(intercept=b[1], slope=b[2], col="#F8766D", size=1)
...
"Visualizing all of the variables in one plot makes it clear that they are pretty closely associated with one another"
...
"just nox has a much lower r^2 though, meaning it explains a lot less of the variance in mortality."
...
first_half <- slice(pollution, 30)

model_e <- lm(mort~log10(nox)*log10(hc)*sqrt(so2), data=first_half)

predict(model_e)

new <- slice(pollution, 30:60)

predict(model_e, new)

"The predict for the second half matching the modle of the first looks similar to the first half. I suspect that the residuals will be similar for both, but I forgot to check and it's due now."
...
ggplot(mc, aes(x=age)) + geom_histogram(bins = 11) + facet_grid(.~sex)
...
"\n\n
Looking at the essays might be more productive. Let's check random words"
...
"I did this for a bunch of others, but it's slow"
...
"Fortunately, python exists. I counted the number of respondents that used a given word, for total, male and female."
...
datatable(w_counts, options = list(searching = TRUE, pageLength = 20))

```
> Now we have a data table of all the most common words, and how often they were used in males and females, and the percent difference between observed and expected
